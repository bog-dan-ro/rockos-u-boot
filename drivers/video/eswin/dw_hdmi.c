...
static int hdmi_phy_configure(struct dw_hdmi *hdmi)
{
    ...
    if (hdmi->edid_data.display_info.hdmi.scdc.supported)
        eswin_dw_hdmi_scdc_set_tmds_rate(hdmi);
...
int eswin_dw_hdmi_init(struct display_state *state)
{
    ...
    const struct eswin_connector *connector = conn_state->connector;
    ...
    conn_state->output_mode = ESWIN_OUT_MODE_AAAA;
...
int eswin_dw_hdmi_get_timing(struct display_state *state)
{
    ...
    const u8 def_modes_vic[6] = {2, 4, 16, 17, 31, 19};
    ...
    *mode = *hdmi->edid_data.preferred_mode;
    ...
    case MEDIA_BUS_FMT_UYYVYY8_0_5X24:
    case MEDIA_BUS_FMT_UYYVYY10_0_5X30:
        conn_state->output_mode = ESWIN_OUT_MODE_YUV420;
        break;
    }
