...
static int eswin_pcie_power_on(struct eswin_pcie *priv)
{
    ...
    if (ret)
        pr_warn("WARNING: cfg_rst deassert failed: %d\n", ret);

    /* pciet_powerup_rstn */
    ret = reset_deassert(priv->powerup_rst);
    if (ret)
        pr_warn("WARNING: powerup_rst deassert failed: %d\n", ret);
...
static int eswin_pcie_init_port(struct udevice *dev)
{
    ...
    if (ret)
        pr_warn("WARNING: perst assert failed: %d\n", ret);
    udelay(100);
    ret = reset_deassert(priv->perst);
    if (ret)
        pr_warn("WARNING: perst deassert failed: %d\n", ret);
