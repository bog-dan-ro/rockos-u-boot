...
static void ahci_fill_cmd_slot(struct ahci_ioports *pp, u32 cmd_slot, u32 opts)
{
	...
	cmd_hdr->status = 0;
	pp->cmd_slot->tbl_addr = cpu_to_le32(lower_32_bits(virt_to_phys(pp->cmd_tbl)));
	pp->cmd_slot->tbl_addr_hi = cpu_to_le32(upper_32_bits(virt_to_phys(pp->cmd_tbl)));
}
...
static int ahci_port_start(struct ahci_uc_priv *uc_priv, u8 port)
{
	...
	writel_with_flush(lower_32_bits(virt_to_phys(pp->cmd_slot)), &port_mmio->clb);
	writel_with_flush(lower_32_bits(virt_to_phys(pp->rx_fis)), &port_mmio->fb);
